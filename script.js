...
(() => {
  ...
  let memory = 0;
  function updateSciVisibility(){
    if (!sciToggle || !sciKeys) return;
  ...
  // Theme
  themeToggle.addEventListener('change', setThemeFromToggle);
    document.body.classList.toggle('light', !themeToggle.checked);
  });

  // Scientific toggle
  const onSciToggle = () => updateSciVisibility();
  sciToggle.addEventListener('change', onSciToggle);
  sciToggle.addEventListener('input', onSciToggle);
